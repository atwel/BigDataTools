...
class FBgraph(object):
    ...
    def setup_db(self):
        ...
        ip.core.display.clear_output(other=False) 
        ...
        while not asked:
            ...
            if self.check_answer(q):
                count +=1
                print "\nPlease enter a place you lived"
                self.contexts_list[count] = raw_input() + " (lived)"
            else:
                asked=True
        ...
        # Clearing the Notebook input screen
        ip.core.display.clear_output(other=False)
    ...
    def extend_contexts_list(self):
        ...
        adding = True
        while adding:
            ip.core.display.clear_output(other=False) # Clearing the Notebook input screen
    ...
    def random_sample(self,count=200):
        ...
        try:
            ...
            print "The sample size is larger than your current network size."
                ...
                while asking:
                    ...
                    try:
                        if (0.<=float(q)<=1.):
                            ...
                            self.mynet[int(person)][self.my_ID]["weight"] = (float(q)*20)+1
                            asking=False
                        else:
    ...
    def code_for_place_met(self, overwrite=False):
        ...
        for person in list(self.friend_db.columns.values):
            ...
            if (self.friend_db[int(person)]["known from"] != self.friend_db[int(person)]["known from"] or overwrite) and person != self.my_ID: # funky NaN check
                print "Where do you know " + str(self.friend_db[int(person)]["name"]) + " from? (Enter a number)"
                for i in self.contexts_list.keys():
    ...
    def add_and_code_new_attribute(self, overwrite=False):
        ...
        if 'undefined1' in indices or 'undefined2' in indices or 'undefined3' in indices: 
            ...
            while asking:
                ...
                attribute = raw_input()
                ip.core.display.clear_output(other=False)
                print "Is "+attribute+ " correct? (y/n)"
            ...
            while asking:
                ...
                if self.check_answer(raw_input()):
                    print "Please name value " +str(count)+":"
                    my_dictionary[count] = raw_input()
                    count +=1
                else:
                    asking=False
            ...
            count = 0
            ip.core.display.clear_output(other=False)
            print "OK, now it's time to code your friends"
            for person in list(self.friend_db.columns.values):
    ...
    def create_gender_dictionary(self):
        print "\nFacebook forces users to choose either female or male"
    ...
    def change_friend_values(self, name):
        ...
        if id == 0:
            ...
            for attr in list(self.friend_db.index.values):
                ...
                print str(name)+ " has value "+ str(val)+ " for attribute "+str(attr)+". Would you like to change it? (y/n)"
                q =raw_input()
                if self.check_answer(q):
                    asking = True
                    while asking:
                        ...
                        if attr == "tie strength":
                            print "Please input the new value:"
                            q = raw_input()
                            try:
                                q = float(q)
                                if 0.<=q<=1.:
    ...
    def load_data(self,file_name="data1"):
    ...
    def diameter(self):
        return nx.diameter(self.mynet)

    def degree_centrality(self, node=True):
    ...
    def bewteenness_centrality(self, node=True):
    ...
    def closeness_centrality(self, node=True):
    ...
    def eigenvector_centrality(self, node=True):
    ...
    def clustering(self, group=0):
        ...
        #rewrite = {}
        #key = self.ids_to_names()
        #for i in clustering:
        #    rewrite[key[int(i)]] = clustering[i]
    ...
    def associativity_by_attribute(self, attribute):
        ...
        if attribute =="id" or attribute =="mutuals":
            ...
            for i in trans_dict:
                ...
                avg_dict[i] = avg
                print "Transitivity avg for people with value "+str(i) + " is: " +str(avg)
